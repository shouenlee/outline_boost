...
def extract_text_from_image(image, lang='eng') -> str:
...
if __name__ == '__main__':
    ...
    if contour is None:
        exit('Counld not contour document')

    print('STEP 3: Apply top down transform')
    warped = apply_top_down_transform(orig, contour, ratio)

    print('STEP 4: Extract text from document')
    text = extract_text_from_image(warped, lang='eng')
    print(text)
    ...
    cv2.imwrite('document.png', warped)